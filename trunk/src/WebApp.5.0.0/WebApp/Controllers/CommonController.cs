namespace com.Sconit.Web.Controllers
{
    ...
    public class CommonController : WebAppBaseController
    {
        ...
        private static string selectEqTaskSubTypeStatement = "from TaskSubType as t where t.Code = ?";
        ...
        public ActionResult _ManufacturePartyDropDownList(string controlName, string controlId, string selectedValue, bool? includeBlankOption, string blankOptionDescription, string blankOptionValue, bool? enable)
        {
            ViewBag.ControlName = controlName;
            ViewBag.ControlId = controlId;
            //ViewBag.SelectedValue = selectedValue;
        ...
        public ActionResult _AjaxLoadingManufactureParty(string text, string item)
        {
            ...
            return PartialView(new SelectList(partyList.OrderBy(p => p.Code), "Code", "CodeDescription"));
        }


        #region newUom
        public ActionResult _NewManufacturePartyDownList(string item, string controlName, string controlId, string selectedValue, bool? includeBlankOption, string blankOptionDescription, string blankOptionValue, bool? enable)
        {
            string hql = "select p from Party p where p.Code in (select distinct  m.PartyFrom from FlowMaster as m where exists (select 1 from FlowDetail as d where d.Flow=m.Code and d.Item='" + item + "') and m.Type in (" + (int)com.Sconit.CodeMaster.OrderType.Procurement + "," + (int)com.Sconit.CodeMaster.OrderType.Distribution + "))";
            IList<Party> partyList = genericMgr.FindAll<Party>(hql);
            return PartialView(new SelectList(partyList.OrderBy(p => p.Code), "Code", "CodeDescription"));
        ...
        #region TaskAddress
        public ActionResult _taskAddressComboBox(string controlName, string controlId, string selectedValue, bool? enable)
        {
            ViewBag.ControlName = controlName;
            ViewBag.ControlId = controlId;
            ViewBag.Enable = enable;
            IList<TaskAddress> taskAddressList = new List<TaskAddress>();
            if (selectedValue != null && selectedValue.Trim() != string.Empty)
            {
                taskAddressList = base.genericMgr.FindAll<TaskAddress>(selectEqTaskAddressStatement, new object[] { selectedValue }, firstRow, maxRow);
            }

            return PartialView(new SelectList(taskAddressList, "Code", "Description", selectedValue));
        }

        public ActionResult _AjaxLoadingTaskAddress(string text)
        {
            IList<TaskAddress> taskAddressList = new List<TaskAddress>();

            string hql = "from TaskAddress p ";
            IList<object> paramList = new List<object>();

            if (!string.IsNullOrEmpty(text))
            {
                hql += "  where p.Code like ? ";
                paramList.Add(text + "%");
            }

            taskAddressList = base.genericMgr.FindAll<TaskAddress>(hql, paramList.ToArray(), firstRow, maxRow);

            return new JsonResult
            {
                Data = new SelectList(taskAddressList, "Code", "Description", "")
            };
        }
        #endregion

        #region TaskSubType
        public ActionResult _taskSubTypeComboBox(string controlName, string controlId, string selectedValue, bool? enable)
        {
            ViewBag.ControlName = controlName;
            ViewBag.ControlId = controlId;
            ViewBag.Enable = enable;
            IList<TaskSubType> taskSubTypeList = new List<TaskSubType>();
            if (selectedValue != null && selectedValue.Trim() != string.Empty)
            {
                taskSubTypeList = base.genericMgr.FindAll<TaskSubType>(selectEqTaskSubTypeStatement, new object[] { selectedValue }, firstRow, maxRow);
            }

            return PartialView(new SelectList(taskSubTypeList, "Code", "Description", selectedValue));
        }

        public ActionResult _AjaxLoadingTaskSubType(string text)
        {
            IList<TaskSubType> taskSubTypeList = new List<TaskSubType>();

            string hql = "from TaskSubType p ";
            IList<object> paramList = new List<object>();
            ...
            taskSubTypeList = base.genericMgr.FindAll<TaskSubType>(hql, paramList.ToArray(), firstRow, maxRow);

            return new JsonResult
            {
                Data = new SelectList(taskSubTypeList, "Code", "Description", "")
            };
        }
        #endregion
