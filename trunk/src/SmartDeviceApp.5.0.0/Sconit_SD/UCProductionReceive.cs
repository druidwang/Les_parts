        ...
        protected override void ScanBarCode()
        {
            ...
            if (this.miscOrderMaster == null && !this.isCancel)
            {
                if (op == CodeMaster.BarCodeType.MIS.ToString())
                {
                    ...
                    {
                        throw new BusinessException("扫描的成品入库单不存在");
                    }
                    else if (miscOrderMaster.Status != MiscOrderStatus.Create)
                    {
                        if (miscOrderMaster.Type == MiscOrderType.GR)
                        {
                            throw new BusinessException("成品入库单{0}状态不是创建", miscOrderMaster.MiscOrderNo);
                        }
                    }
                    else if (!miscOrderMaster.IsScanHu)
                    {
                        ...
                        if (miscOrderMaster.Type == MiscOrderType.GR)
                        {
                            throw new BusinessException("成品入库单{0}不需要扫描物料条码", miscOrderMaster.MiscOrderNo);
                        }
                    }
                    else
                    {
                ...
                {
                    throw new BusinessException("请先扫描成品入库单");
                }
            }
