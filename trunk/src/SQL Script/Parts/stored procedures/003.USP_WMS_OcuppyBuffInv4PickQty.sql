...
BEGIN
	...
	begin try
		...
		begin
			truncate table #tempOccupyBuffInv_003
		end
		...
		begin try
			...
			--获取发货计划
			insert into #tempShipPlan_003(ShipPlanId, [Priority], StartTime, LocFrom, Item, UOM, UC, UnitQty, 
			...
			begin
				...
				select @Loc = Loc, @Item = Item, @Qty = Qty from #tempBuffInv_003 where RowId = @RowId
	...
	insert into #tempOccupyBuffInv_003(Id, PickedQty) select ShipPlanId, ThisPickedQty from #tempShipPlan_003 where ThisPickedQty > 0
