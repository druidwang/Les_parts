...
BEGIN
	...
	begin try
		...
		begin
			...
			begin --按条码拣货
				if (@PickBy = 0)
				begin  --按批号拣货
					set @SelectInvStatement = 'select sp.Location, sp.Item, hu.UC, hu.LotNo, bin.Area, llt.Bin, bin.Seq, SUM(llt.Qty) as InvQty '
					set @SelectInvStatement = @SelectInvStatement + 'from #tempPickTarget_002 as sp '
					set @SelectInvStatement = @SelectInvStatement + 'inner join INV_LocationLotDet_' + @LocSuffix + ' as llt on sp.Location = llt.Location and sp.Item = llt.Item '
					...
					set @SelectInvStatement = @SelectInvStatement + 'group by sp.Location, sp.Item, hu.UC, hu.LotNo, bin.Area, llt.Bin, bin.Seq'
				
					insert into #tempAvailableInv_002(Location, Item, UC, LotNo, Area, Bin, BinSeq, Qty) exec sp_executesql @SelectInvStatement
				end
				else
				begin  --按条码拣货
					set @SelectInvStatement =  'select sp.Location, hu.UC, sp.Item, hu.HuId, hu.LotNo, bin.Area, llt.Bin, bin.Seq, llt.Qty '
					set @SelectInvStatement = @SelectInvStatement + 'from #tempPickTarget_002 as sp '
					set @SelectInvStatement = @SelectInvStatement + 'inner join INV_LocationLotDet_' + @LocSuffix + ' as llt on sp.Location = llt.Location and sp.Item = llt.Item ' 
					set @SelectInvStatement = @SelectInvStatement + 'inner join INV_Hu as hu on llt.HuId = hu.HuId '
					set @SelectInvStatement = @SelectInvStatement + 'inner join MD_LocationBin as bin on llt.Bin = bin.Code '
					...
					insert into #tempAvailableInv_002(Location, Item, UC, HuId, LotNo, Area, Bin, BinSeq, Qty) exec sp_executesql @SelectInvStatement
				end
			end
		...
		begin --按条码拣货
			...
			begin  --按批号拣货
				...
				update inv set Qty = inv.Qty - oc.OccupyQty
				from #tempAvailableInv_002 as inv inner join
				(select pt.Loc, pt.Item, pt.UC, pt.LotNo, pt.Bin, SUM((pt.OrderQty - pt.PickQty) * pt.UnitQty) as OccupyQty
				from #tempPickTarget_002 as sp 
				inner join WMS_PickTask as pt on sp.Location = pt.Loc and sp.Item = pt.Item
				where pt.IsPickHu = 1 and pt.PickBy = 0 and pt.IsActive = 1 and pt.OrderQty > pt.PickQty and pt.Bin is not null
				group by pt.Loc, pt.Item, pt.UC, pt.LotNo, pt.Bin) as oc on inv.Location = oc.Loc and inv.Item = oc.Item and inv.UC = oc.UC and inv.LotNo = oc.LotNo and inv.Bin = oc.Bin
	...
	select Location, Item, UC, Area, Bin, BinSeq, HuId, LotNo, Qty from #tempAvailableInv_002
