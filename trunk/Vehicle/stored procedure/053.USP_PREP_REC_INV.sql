...
BEGIN
	...
	BEGIN TRY
		IF NOT EXISTS(SELECT TOP 1 1 FROM tempdb.sys.objects WHERE type = 'U' AND name LIKE '#TMP_REC_INV_INPUT')
		BEGIN
			SET @ERROR_MSG = '没有定义临时表#TMP_REC_INV_INPUT表。'
			RAISERROR(@ERROR_MSG, 16, 1) 

			--
			CREATE TABLE #TMP_REC_INV_INPUT (
				...
				IS_TR BIT,
				IS_CNMT BIT,
				INV_QTY DECIMAL(18, 8),
				VER INT
			)
		END

		IF NOT EXISTS(SELECT TOP 1 1 FROM #TMP_REC_INV_INPUT)
		BEGIN
			--采购收货记录为空
