...
BEGIN
	...
	BEGIN TRY
		...
		IF NOT EXISTS(SELECT TOP 1 1 FROM #TMP_REC_ORD_INPUT)
		BEGIN
			--采购收货记录为空
			SET @ERROR_MSG = N'采购收货记录不能为空。'
			RAISERROR(@ERROR_MSG, 16, 1) 
		END
		ELSE BEGIN
			--查找价格单
			...
			SELECT A.VOU_LN, ROW_NUMBER() OVER(PARTITION BY B.PUR_PL_ID ORDER BY B.CRT_DT) AS SUB_SEQ, A.ORD_LN, A.DLV_LN, B.PUR_PL_ID, B.SUPPL_CD,
			B.PT_NO, A.QTY, B.UOM, B.BASE_UOM, B.UOM_CONV_RT, B.CUR, B.UNIT_PRICE, B.IS_PROV_EST, B.TAX_CD, B.BIL_TERM
			...
			--把寄售物料记录到寄售物料临时表里
			INSERT INTO #TMP_REC_CNMT_INPUT(VOU_LN, ORD_LN, DLV_LN, PUR_PL_ID, SUPPL_CD, 
								PT_NO, UOM, BASE_UOM, UOM_CONV_RT, CNMT_QTY)
			SELECT VOU_LN, ORD_LN, DLV_LN, PUR_PL_ID, SUPPL_CD, 
